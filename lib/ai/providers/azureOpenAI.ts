...
export class AzureOpenAIProvider implements AIProvider {
  ...
  async generateProduct(request: GenerateProductRequest): Promise<GeneratedProduct> {
    ...
    try {
      ...
      throw new Error('Failed to generate product with Azure OpenAI');
    }
  }
...
Return only valid JSON.`;
    ...
    try {
      ...
      console.error('Azure OpenAI trend analysis error:', error);
      return [];
    }
  }

  async generateImage(prompt: string): Promise<string> {
    // Azure OpenAI doesn't have image generation in the same way
    // This would need to be implemented with DALL-E or another service
...
Return only the content, no explanations.`;
    ...
    try {
      ...
      console.error('Azure OpenAI listing generation error:', error);
